...
Left : https://www.geeksforgeeks.org/problems/rotate-array-by-n-elements-1587115621/1
Right : https://leetcode.com/problems/rotate-array/
...
```cpp
// 🔵 Optimal: Reversal Algorithm - Left Rotation
void leftRotate(vector<int>& arr, int d) {
    int n = arr.size();
    d = d % n; // Handle d > n
    if (d == 0) return;
...
// 🔵 Optimal: Reversal Algorithm - Right Rotation
void rightRotate(vector<int>& arr, int d) {
    int n = arr.size();
    d = d % n; // Handle d > n
    if (d == 0) return;
...
- `O(n)`: Three reversals: O(d) + O(n-d) + O(n) ≈ O(n).
...
- **Brute Force**: `O(n)`, `O(d)` – Simple save-shift-place, but uses extra space.
- **Reversal**: `O(n)`, `O(1)` – Optimal, in-place, elegant block swapping.
...
- **Practice**:
    - Left: `[1,2,3,4,5,6,7], d=3` → `[4,5,6,7,1,2,3]`
    - Right: `[1,2,3,4,5,6,7], d=3` → `[5,6,7,1,2,3,4]`
- **Related Problems**:
    - [LeetCode 189: Rotate Array](https://leetcode.com/problems/rotate-array/)
    - [GeeksforGeeks: Cyclically Rotate an Array](https://www.geeksforgeeks.org/cyclically-rotate-an-array-by-one/)
